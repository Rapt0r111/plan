// features/timeline/ui/EpicModal.tsx
"use client";
...
interface Props { epic: EpicWithTasks; onClose: () => void; }
    ...
    <AnimatePresence>
      <motion.div key="overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
        ...
        <motion.div key="panel" initial={{ opacity: 0, scale: 0.94, y: 20 }}
          ...
          className="w-full flex flex-col" onClick={(e) => e.stopPropagation()}
          style={{ maxWidth: 640, maxHeight: "88vh", background: "var(--bg-elevated)",
            border: "1px solid var(--glass-border)", borderRadius: 20,
            boxShadow: `0 32px 80px rgba(0,0,0,0.6), 0 0 0 1px ${liveEpic.color}18` }}>

          {/* Header */}
          <div className="shrink-0 px-5 pt-5 pb-4"
            style={{ borderBottom: "1px solid var(--glass-border)", borderRadius: "20px 20px 0 0",
              background: `linear-gradient(135deg, ${liveEpic.color}0e 0%, transparent 55%)` }}>
            <div className="flex items-start gap-3">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 text-base font-bold font-mono"
                style={{ background: `${liveEpic.color}1a`, border: `1.5px solid ${liveEpic.color}45`, color: liveEpic.color }}>
                {liveEpic.title.charAt(0).toUpperCase()}
              </div>
              <div className="flex-1 min-w-0">
                <h2 className="text-base font-semibold" style={{ color: "var(--text-primary)" }}>{liveEpic.title}</h2>
                {liveEpic.description && (
                  <p className="text-xs mt-0.5 line-clamp-2" style={{ color: "var(--text-muted)" }}>{liveEpic.description}</p>
                )}
                {liveEpic.startDate && (
              ...
              {liveEpic.progress.total > 0 && (
                <div className="flex h-1 mt-1.5 rounded-full overflow-hidden gap-px">
                  {STATUS_ORDER.map((s) => {
                    ...
                    if (!n) return null;
                    return <motion.div key={s} layout title={`${S[s].label}: ${n}`}
          ...
          {/* Filter tabs */}
          <div className="shrink-0 flex items-center gap-1.5 px-4 py-2.5 overflow-x-auto"
            ...
            {([["all", `Все · ${liveEpic.tasks.length}`, liveEpic.color] as const,
              ...
              return (
                <button key={key} onClick={() => setFilter(key as typeof filter)}
                  ...
                  style={{ background: active ? `${color}20` : "var(--glass-01)",
            ...
            <LayoutGroup id={`epic-tasks-${liveEpic.id}`}>
              ...
              <AnimatePresence>
                {archivedTasks.length > 0 && (
                  <motion.div key="archive" layout initial={{ opacity: 0, height: 0 }}
          ...
          {/* Footer */}
          <div className="shrink-0 px-5 py-3 flex justify-between items-center"
            style={{ borderTop: "1px solid var(--glass-border)", borderRadius: "0 0 20px 20px",
            ...
            <button onClick={onClose}
              ...
              style={{ background: `${liveEpic.color}18`, color: liveEpic.color, border: `1px solid ${liveEpic.color}30` }}>
              Закрыть
            </button>
          </div>
        </motion.div>
