    ...
    <div className="rounded-2xl overflow-hidden flex flex-col"
      ...
      <div className="flex items-center gap-3 px-5 py-3 shrink-0"
        ...
        <div className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0"
          ...
          <svg className="w-4 h-4" style={{ color: "#8b5cf6" }} viewBox="0 0 16 16"
        ...
        <div className="hidden sm:flex items-center gap-2 shrink-0">
          <div className="w-28 h-1.5 rounded-full overflow-hidden" style={{ background: "var(--glass-02)" }}>
            <motion.div className="h-full rounded-full" animate={{ width: `${overallPct}%` }}
        ...
        <div className="hidden md:flex items-center gap-3 shrink-0">
          {STATUS_ORDER.map((s) => (
            <div key={s} className="flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: S_COLOR[s] }} />
              <span className="text-[10px] font-mono" style={{ color: "var(--text-muted)" }}>{S_LABEL[s]}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div ref={scrollRef} className="relative overflow-auto"
        style={{ maxHeight: MAX_AREA_H, cursor: isDrag ? "grabbing" : "grab",
          userSelect: "none", scrollbarWidth: "thin", scrollbarColor: "rgba(255,255,255,0.08) transparent" }}
        ...
        <div style={{ minWidth: LEFT_W + layout.canvasW, position: "relative" }}>
          ...
          <div className="absolute pointer-events-none"
            ...
            {layout.months.map(({ x, isJan }) => (
              <div key={`vl-${x}`} className="absolute top-0 bottom-0 w-px"
                style={{ left: x, background: isJan ? "rgba(255,255,255,0.07)" : "rgba(255,255,255,0.03)" }} />
            ))}
          ...
          {layout.bars.map((bar, idx) => (
            <TimelineRow key={bar.epic.id} bar={bar} index={idx}
      ...
      {/* Footer */}
      <div className="shrink-0 flex items-center gap-4 px-5 py-2"
