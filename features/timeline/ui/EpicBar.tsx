...
export function EpicBar({ bar, hovered, onHover, onClick }: Props) {
  ...
  return (
    <motion.div
      ...
      style={{
        left: barX, width: barW, height: LANE_H - 20, cursor: "pointer",
        background: hovered ? `${accentColor}1c` : `${accentColor}0e`,
        border: `1px solid ${accentColor}${hovered ? "55" : "28"}`,
        pointerEvents: "auto",
      }}
      animate={{ boxShadow: hovered ? `0 0 20px ${accentColor}28, 0 4px 16px rgba(0,0,0,0.3)` : "none" }}
    ...
    >
      ...
      {pct > 0 && (
        <div className="absolute inset-y-0 left-0 rounded-l-lg"
          style={{ width: `${donePct}%`, background: `${accentColor}1a`,
            borderRight: `1px solid ${accentColor}30`, transition: "width 0.5s ease" }} />
      )}
      ...
      {hovered && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          ...
          <span className="text-[11px] font-medium font-mono" style={{ color: accentColor }}>подробнее</span>
          <svg className="w-3 h-3" style={{ color: accentColor }} viewBox="0 0 12 12"
