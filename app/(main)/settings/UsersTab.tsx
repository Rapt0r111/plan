...
export function UsersTab({ initialUsers, roles, personnelGroups }: Props) {
    ...
    const handleForcePasswordChange = useCallback(async (user: UserWithMeta) => {
        const tempPassword = window.prompt(
            `Введите временный пароль для ${user.name}`,
            `Tmp-${crypto.randomUUID().slice(0, 8)}!`
        );

        if (!tempPassword) return;
        if (tempPassword.length < 8) {
            setError("Временный пароль должен содержать не менее 8 символов");
            return;
        }
        ...
        try {
            ...
            const json = await res.json();
            if (!res.ok) {
                setError(json.error ?? "Ошибка принудительной смены пароля");
                return;
            }

            setNotice(`Для ${user.name} установлен временный пароль и запланирован сброс при следующем входе.`);
        } catch {
            setError("Сетевая ошибка");
        }
    }, []);
