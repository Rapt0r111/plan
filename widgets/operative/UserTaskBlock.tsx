...
function QuickAddForm({ accentColor, onAdd, onCancel }: {
  ...
  return (
    <motion.div
      ...
      className="rounded-xl overflow-hidden mx-2.5 mb-2"
  ...
  return (
    ...
    >
      ...
      <div className="px-4 py-3 flex items-start gap-3"
        ...
        {/* Block drag handle — admin only, reorder blocks */}
        {isAdmin && dragHandleProps && (
          <button
            {...dragHandleProps}
            className="shrink-0 mt-1.5 w-5 h-5 flex items-center justify-center opacity-25 hover:opacity-60 cursor-grab active:cursor-grabbing transition-opacity"
        ...
        <div className="flex items-center gap-1 shrink-0 mt-0.5">
          ...
          <motion.button
            onClick={() => { setCollapsed(v => !v); if (!collapsed) { setAddingFromHeader(false); setAddingFromFooter(false); } }}
          ...
          >
            <motion.svg
              viewBox="0 0 12 12" className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"
      ...
      <AnimatePresence>
        {addingFromHeader && !collapsed && (
          ...
          >
            <div className="pt-2">
              <QuickAddForm
                userId={user.id} accentColor={accentColor}
                onAdd={handleAdd}
                onCancel={() => setAddingFromHeader(false)}
              />
            </div>
          </motion.div>
        )}
